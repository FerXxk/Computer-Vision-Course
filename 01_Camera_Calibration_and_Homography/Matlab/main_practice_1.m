...
%% Renderizado mesa
Im_mesa = imread(fullfile(scriptDir, '..', 'repository', 'wooden_table_grayscale.png'));
fondo_2=mp4(:,1:4);
mesa = [0 626 626  0 ;
    0  0  417 417];
...
aruco = [0 756 756  1 ;
    0  0  756 756];
Hc1(:,:)=HomographySolve(aruco,cubo_1);
%Matriz de homografía
Hc2(:,:)=HomographySolve(aruco,cubo_2); %Matriz de homografía
Hc3(:,:)=HomographySolve(aruco,cubo_3);
%Matriz de homografía
Hc4(:,:)=HomographySolve(aruco,cubo_4);
%Matriz de homografía
...
Im_mesa = imread(fullfile(scriptDir, '..', 'repository', 'wooden_table_color.png'));
